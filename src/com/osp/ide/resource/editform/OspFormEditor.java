...
public class OspFormEditor extends GraphicalEditorWithFlyoutPalette implements
		FrameConst {
	protected class OutlinePage extends ContentOutlinePage {
		...
		public void init(IPageSite pageSite) {
			...
			// ContextMenu
			ContextMenuProvider provider = new OspFormContextMenuProvider(
	...
	public static final String ID = "com.osp.ide.resource.editor.frame.OspFormEditor";

	private OutlinePage outline;
	private DrawCanvas model;
	private KeyHandler keyHandler;
	public OspResourceManager m_frame;
	public OspUIString m_string;
	public OspForm m_scene;
	public String m_id = "";
	...
	protected void setInput(IEditorInput input) {
		...
		m_id = input.getName();
		m_frame = ((OspFormEditorInput) input).frame;
		m_string = ((OspFormEditorInput) input).string;
		screen = ((OspFormEditorInput) input).screen;
		Hashtable<String, OspForm> scenes = m_frame.m_Form.get(screen);
		if(scenes != null)
			m_scene = scenes.get(m_id);
		super.setInput(input);
		setPartName("Form[" + screen + "]: " + m_id);
	...
	public void setId(String name) {
		...
		setPartName("Form[" + screen + "]: " + m_id);
		setTitleToolTip(m_id + screen);
		OspFormEditorInput input = (OspFormEditorInput) getEditorInput();
		if(input != null)
			input.ID = m_id;
		if(frameModel != null)
			frameModel.setName(m_id);
	}

	@Override
	protected CustomPalettePage createPalettePage() {
		// TODO Auto-generated method stub
		return super.createPalettePage();
	}
...
//	    	AbstractUIPlugin.imageDescriptorFromPlugin(Activator.PLUGIN_ID, "icons/button.png"));
		...
		CombinedTemplateCreationEntry term = new CombinedTemplateCreationEntry("Button",
				"", Button.class, new OspNodeCreationFactory(
						Button.class), AbstractUIPlugin
		...
		term = new CombinedTemplateCreationEntry("Edit Time", "",
		    EditTime.class, new OspNodeCreationFactory(EditTime.class), AbstractUIPlugin
						.imageDescriptorFromPlugin(Activator.PLUGIN_ID,
	...
	public void doSave(IProgressMonitor monitor) {
		m_frame.SaveFormXML(screen, m_scene.m_infoScene.Id);
		m_string.SaveAll();
	...
	public boolean isDirty() {
		return getCommandStack().isDirty() || m_scene.isDirty();
	}
	...
	public DrawCanvas CreateDrawCanvas() {
		...
		frameModel = new Form(this, m_scene, frameID, getModeIndex(mode));
		ary = new Vector<String>(m_scene.getIdList(frameModel.getItem().children));

		createControls(frameModel, ary);

		drawcanvas.addChild(frameModel, false);
		return drawcanvas;
	}
	...
	private void createControls(FrameNode parent, Vector<String> ary) {
		...
		for (int i = 0; i < ary.size(); i++) {
			if ((item = m_scene.m_items.get(ary.get(i))) != null) {
				FrameNode child = createControls(parent, item, ary.get(i));
				
				if(child != null)
					createControls(child, m_scene.getIdList(child.getItem().children));
			}
		}
	}
	...
	private FrameNode createControls(FrameNode parent, ITEM_INFO item, String id) {
		...
		case WINDOW_OVERLAYPANEL:
			node = new OverlayPanel(
					id, (OVERLAYPANEL_INFO) item);
			break;
		case WINDOW_PANEL:
	...
	protected void initializeGraphicalViewer() {
		...
		verticalRuler.setFrame(frameModel);
		...
		horizontalRuler.setFrame(frameModel);
		...
		if(zoomManager != null)
		    zoomManager.setFrame(frameModel.getFigure());
...
//      InternalGEFPlugin.getDefault().getImageRegistry().put(InternalImages.IMG_PALETTE, icon);
	...
	protected void configureGraphicalViewer() {
		...
		// ContextMenu
		ContextMenuProvider provider = new OspFormContextMenuProvider(viewer,
				getActionRegistry());
		viewer.setContextMenu(provider);

		viewer.setProperty(SnapToGrid.PROPERTY_GRID_ENABLED, true);
		viewer.setProperty(SnapToGrid.PROPERTY_GRID_VISIBLE, true);
		// grid dimension setting
	...
	public void createActions() {
		...
		action = new OspAddClassAction(this);
		registry.registerAction(action);
		getSelectionActions().add(action.getId());
	...
	public void setGridDimension(int dim) {
		...
		if (model != null)
			((Form) model.getChildrenArray().get(0)).setDimension(dim);
...
//				.setZoom(zoom);
	...
	public void ReDrawFrame() {
		Form frame = (Form) model.getChildrenArray().get(0);
		int mode = getModeIndex(this.mode);
		
		reDrawControls(frame, mode);
	...
	private void reDrawControls(FrameNode model, int mode) {
		...
		for (int i = 0; i < children.size(); i++) {
			...
			if(child != null)
				reDrawControls(child, mode);
//			newRect = children.get(i).getLayout(mode);
//			if (mode == PORTRAIT)
//				oldRect = children.get(i).getLayout(LANDCAPE);
//			else
//				oldRect = children.get(i).getLayout(PORTRAIT);
//			children.get(i).setMode(mode);
//			children.get(i).getListeners().firePropertyChange(
//					FrameNode.PROPERTY_LAYOUT, oldRect, newRect);
		}
	...
	public static void refreshPanelEditor() {
		IWorkbenchPage page = PlatformUI.getWorkbench()
				.getActiveWorkbenchWindow().getActivePage();
		IEditorReference[] editors = page.getEditorReferences();
		for (int n = 0; n < editors.length; n++) {
			if (editors[n] != null) {
				if(editors[n].getId().equals(OspPanelEditor.ID)) {
					((OspPanelEditor) editors[n].getEditor(false)).getModel()
					.refreshChildren();
					((OspPanelEditor) editors[n].getEditor(false)).fireDirty();
				}
			}
		}
	}
	...
	public Boolean isGrid() {
		return frameModel.getFigure().getPaintGrid();
	}
	...
	public void setGridVisible(Boolean gridVisible) {
		...
		frameModel.getFigure().paintGrid(gridVisible);
	}
