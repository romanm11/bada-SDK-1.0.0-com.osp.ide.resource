...
public class OspPanelEditor extends GraphicalEditorWithFlyoutPalette implements
		FrameConst {
	protected class OutlinePage extends ContentOutlinePage {
		...
		public void init(IPageSite pageSite) {
			...
			// ContextMenu
			ContextMenuProvider provider = new OspPanelContextMenuProvider(
	...
	public static final String ID = "com.osp.ide.resource.editor.panel.OspPanelEditor";
	...
	protected void setInput(IEditorInput input) {
		...
		m_id = input.getName();
		m_screen = ((OspPanelEditorInput) input).screen;
		m_frame = ((OspPanelEditorInput) input).frame;
		m_string = ((OspPanelEditorInput) input).string;
		Hashtable<String, OspPanel> panels = m_frame.m_Panels.get(m_screen);
		m_OspPanel = panels.get(m_id);
		// System.out.println("setInput] IEditorInput input.getName = "+m_id+", TooTip = "+s);
		super.setInput(input);
		setPartName("ScrollPanel:[" + m_screen + "]: " + m_id);
	}

	public void setId(String id) {
		m_id = id;
		super.setPartName("ScrollPanel:[" + m_screen + "]: " + m_id);
		setTitleToolTip(m_id);
		OspPanelEditorInput input = (OspPanelEditorInput) getEditorInput();
		if(input != null)
			input.ID = m_id;
		if(panelModel != null)
			panelModel.setName(id);
	}
...
//		commandDrawer.add(term);
	...
	public boolean isDirty() {
		return getCommandStack().isDirty() || m_OspPanel.isDirty();
	}
	...
	public DrawCanvas CreateDrawCanvas() {
		...
		for (int i = 0; i < editors.length; i++) {
			...
			if (editor instanceof OspPanelEditor) {
				mode = ((OspPanelEditor) editor).mode;
				break;
			}
		}
		panelModel = new PanelFrame(this, m_OspPanel, panelID, getModeIndex(mode));

		ary = m_OspPanel.getIdList();

		createControls(ary);

		drawcanvas.addChild(panelModel, false);
		return drawcanvas;
	}

	private void createControls(Vector<String> ary) {
		ITEM_INFO item;
		for (int i = 0; i < ary.size(); i++) {
			if ((item = m_OspPanel.m_items.get(ary.get(i))) != null) {
				createControls(panelModel.getContainer(item.pID), item, ary.get(i));
			}
		}
	}

	private void createControls(FrameNode parent, ITEM_INFO item, String id) {
		FrameNode node = null;
		switch (item.type) {
		case WINDOW_BUTTON:
			node = new Button(id,
	...
	protected void initializeGraphicalViewer() {
		...
		verticalRuler.setFrame(panelModel);
		...
		horizontalRuler.setFrame(panelModel);
		...
		if(zoomManager != null)
		    zoomManager.setFrame(panelModel.getFigure());
...
//      InternalGEFPlugin.getDefault().getImageRegistry().put(InternalImages.IMG_PALETTE, icon);
	...
	protected void configureGraphicalViewer() {
		...
		// ContextMenu
		ContextMenuProvider provider = new OspPanelContextMenuProvider(viewer,
				getActionRegistry());
	...
	public void createActions() {
		...
		action = new OspPasteNodeAction(this, "");
		registry.registerAction(action);
		getSelectionActions().add(action.getId());
	...
	public void setGridDimension(int dim) {
		...
		if (model != null)
			((PanelFrame) model.getChildrenArray().get(0)).setDimension(dim);
	}
	...
	public void ReDrawFrame() {
		PanelFrame panel = (PanelFrame) model.getChildrenArray().get(0);
		int mode = getModeIndex(this.mode);
		
		reDrawControls(panel, mode);
	...
	private void reDrawControls(FrameNode model, int mode) {
        ...
        if(model instanceof PanelFrame)
        	model.reSize(mode);
		...
		for (int n = 0; n < editors.length; n++) {
			if (editors[n] != null) {
				if(editors[n].getId().equals(OspFormEditor.ID)) {
					...
					.refreshChildren();
					((OspFormEditor) editors[n].getEditor(false)).fireDirty();
				} else if(editors[n].getId().equals(OspPopupEditor.ID)) {
                    ((OspPopupEditor) editors[n].getEditor(false)).getModel()
                    .refreshChildren();
                    ((OspPopupEditor) editors[n].getEditor(false)).fireDirty();
                }
			}
		}
	}
	...
	public Boolean isGrid() {
		return panelModel.getFigure().getPaintGrid();
	}
	...
	public void setGridVisible(Boolean gridVisible) {
		...
		panelModel.getFigure().paintGrid(gridVisible);
	}
