...
public class SlidableGroupedListPropertySource extends OspNodePropertySource {

	public SlidableGroupedListPropertySource(SlidableGroupedList node) {
		this.node = node;

		SLIDABLEGROUPEDLIST_INFO info = node.getItem().clone();
		this.info = info;
	...
	protected void initDescriptor() {
        ...
        PropertyDescriptor TextColorDescriptor = (new ResourceColorPropertyDescriptor(FrameNode.PROPERTY_COLOROFEMPTYLISTTEXT, "Color Of Empty List Text"));
...
//		properties.add(ModeDescriptor);
	...
	public Object getPropertyValue(Object id) {
		SlidableGroupedList node = (SlidableGroupedList) this.node;
		String text = "";
	...
	public void resetPropertyValue(Object id) {
		SLIDABLEGROUPEDLIST_INFO info = (SLIDABLEGROUPEDLIST_INFO) this.info;
		SlidableGroupedList node = (SlidableGroupedList) this.node;
		...
		if (id.equals(FrameNode.PROPERTY_RENAME)) {
			...
			try {
				...
				Activator.setErrorMessage(
						"SlidableGroupedListPropertySource.setPropertyValue()",
						id + " NumberFormatException - " + e.getMessage(), e);
			}
		} else if (id.equals(FrameNode.PROPERTY_POINTY)) {
			try {
				Layout rect = node.getLayout();
